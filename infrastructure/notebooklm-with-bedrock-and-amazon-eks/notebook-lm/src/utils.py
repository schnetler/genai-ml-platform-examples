...
def text_to_speech_kokoro(text, voice_id, kokoro_client):
    """Convert text to speech using Kokoro TTS"""
    try:
        with kokoro_client.audio.speech.with_streaming_response.create(
            model="kokoro",
            voice=voice_id, #single or multiple voicepack combo
            input=text
        ) as response:
            # Read the streaming response into a BytesIO buffer
...
def create_dialogue_audio(dialogue, output_file, tts_model):
    """Convert dialogue to speech and save to file"""

    # Retrieve kokoro service endpoint from environment variable, if configured
    # This will be localhost for dev, and kokoro_service if running on EKS
    kokoro_endpoint = os.environ.get("KOKORO_ENDPOINT")

    # Initialize TTS agents and set voices
    if tts_model == "Amazon Polly":
    ...
    # else if kokoro
    elif tts_model == "Kokoro TTS on Amazon EKS":
        # Set Kokoro endpoint
        kokoro_client = OpenAI(
            base_url=f"http://{kokoro_endpoint}:8880/v1", api_key="not-needed"
        )

        speaker_voices_kokoro = {
            "Guest": "am_liam+am_puck",  # Male voice
            "Host": "af_sky+af_bella",  # Female voice
        }
    ...
    for line in dialogue:
        ...
        # if and elif for polloy and kokoro
        if tts_model == "Amazon Polly":
            audio_segment = text_to_speech(text, speaker_voices[speaker], polly_client)
        elif tts_model == "Kokoro TTS on Amazon EKS":
            audio_segment = text_to_speech_kokoro(text, speaker_voices_kokoro[speaker], kokoro_client)
