...
from utils import scan_model_configs
from utils import unwrap_model
from utils import print_metric_data
import open_clip
...
def main(args):
    ...
    cfgs = scan_model_configs()
    open_clip.factory._MODEL_CONFIGS.update(cfgs)
    ...
    if args.logit_scale:
        model.logit_scale.data.fill_(np.log(args.logit_scale))
    if args.logit_bias:
        if model.logit_bias:
            model.logit_bias.data.fill_(args.logit_bias)
        else:
            model.logit_bias = torch.nn.Parameter(torch.ones([]) * args.logit_bias)
            model.logit_bias.data = model.logit_bias.data.to(model.logit_scale.device)
    ...
    if args.distributed and not args.horovod:
        ...
        model  = neuronx_distributed.pipeline.NxDPPModel(
            model,
            transformer_layer_cls=ResidualAttentionBlock,
            num_microbatches=32,
            virtual_pipeline_size=2,
            auto_partition=True,
            trace_file_path='./cache',
            use_zero1_optimizer=False,
            deallocate_pipeline_outputs=False,
            output_loss_value_spec=(False, False, False),
            input_names=["image", "text", "batch_size"])
    ...
    if args.train_data or args.dataset_type == "synthetic":
        ...
        scaler = None#GradScaler() if args.precision == "amp" else None
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        #print_metric_data("epochTime", int(time.time() - time_epoch_started))
        ...
        if args.save_logs:
            checkpoint_dict = {
                ...
                "name": args.name,
                "state_dict": {key: value for key, value in unwrap_model(model).state_dict().items() if 'logit_bias' not in key},
            ...
            if completed_epoch == args.epochs or (
                ...
                #)
                xm.save(checkpoint_dict, f"epoch_{completed_epoch}.pt")
            ...
            if args.save_most_recent:
                ...
                #neuronx_distributed.parallel_layers.save(checkpoint_dict, tmp_save_path)
                xm.save(checkpoint_dict, tmp_save_path)
