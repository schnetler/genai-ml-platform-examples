...
def text_global_pool(x, text: Optional[torch.Tensor] = None, pool_type: str = 'argmax', batch_size=None):
    if pool_type == 'first':
        pooled, tokens = x[:, 0], x[:, 1:]
    elif pool_type == 'last':
        ...
        if batch_size is not None:
            pooled, tokens = x[torch.arange(batch_size), text.argmax(dim=-1)], x
        else:
